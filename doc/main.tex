...
% TEMPORARY if visit_int is no longer provided by default,
% we will need to define it
% if we keep it, we should explain that it fails when the two integers
% are different
...
(\sref{sec:ancestors}). For instance, the \oc|visit_list| methods provided by
...
These function definitions differ slightly in that, in the case of \oc|strip|,
a single immutable visitor object is created initially and serves in every
call to \oc|strip|, whereas, in the case of \oc|number|, a new visitor object
is created every time \oc|number| is called. The latter style should be used
when the visitor object has mutable state.
