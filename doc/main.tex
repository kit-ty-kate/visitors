...
\input{version}
% Let our version number appear at the bottom right of every page.
\makeatletter
\def\@formatyear{\visitorsversion}
\makeatother
...
\section{Advanced examples} % TEMPORARY ou: applications, illustrations?
\label{sec:advanced}
...
\begin{figure}[t]
\orig{expr12}
\caption{Open and closed types of arithmetic expressions}
\label{fig:expr12}
\end{figure}
...
\oc|'expr oexpr| has subexpressions of type \oc|'expr|. The closed (recursive)
type of expressions, \oc|expr|, can then be defined as the fixed point of the
functor \oc|oexpr|.
% TEMPORARY citation; demander à Pierre-Évariste

% TEMPORARY ICI commenter la \fref{fig:expr12}.

% TEMPORARY vérifier que l'on sait programmer une fonction map pour le type
% ouvert oexpr; vérifier aussi si l'on sait définir deux points fixes
% différents (e.g. l'un sans info, l'autre avec) et mapper de l'un vers
% l'autre.
