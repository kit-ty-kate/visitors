...
% TEMPORARY can re-use the same examples to illustrate freeze & final
...
named \oc|Foo| can be found and offers these four functions. The \opath
option (\sref{sec:path}) lets the user control where the module \oc|Foo|
is found.
...
référence:
  document the option \opath \label{sec:path}
  document the regularity restriction \label{sec:regularity} and the option irregular
  document the option freeze (expr09/expr10)
  document the option final
