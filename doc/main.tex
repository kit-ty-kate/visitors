...
the second part of \fref{fig:expr00}.
...
it calls the method \tyconvisitor{int}, which is inherited from
\oc|VisitorsRuntime.iter| and does
...
% TEMPORARY the default map visitor on an immutable data structure
% is the identity, so it is uninteresting;
% but the default visitor on a mutable data structure is a deep copy,
% which can be useful.
...
list which types are unsupported
...
documenter endo
  like map, but avoids re-allocating if possible
  makes sense for immutable data structures only
    but the package does not enforce that, e.g. it will let you
    create an endo visitor for a mutable record
